...
This repository is used to create a static webpage via GitHub pages for a book club. The page is generated on the basis of the contents of `data/books.json` and `club.json`. It defines GitHub Actions and GitHub Issue templates to add and review books without the need to code. The `add-book` action runs a Python script that queries the [Open Library](https://openlibrary.org/) API, which allows to fetch book metadata via ISBN or book-title. 
